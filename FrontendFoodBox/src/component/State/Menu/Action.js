...
export const getMenuItemsByRestaurantId = (reqData) => {
    return async (dispatch) => {
        ...
        try {
            // Ensure valid boolean values for the API call
            ...
            // Make the API request with corrected query parameters
            const { data } = await api.get(
                `api/food/restaurant/${reqData.restaurantId}?vegetarian=${vegetarian}&nonveg=${nonveg}&seasonal=${seasonal}&food_category=${foodCategory}`,
                {
                    headers: {
